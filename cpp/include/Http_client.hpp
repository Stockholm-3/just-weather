...
public:
    ...
    // Construct with host and optional port for convenience; request(path) will use these
    HttpClient(const std::string& host, uint16_t port = 80);
    ~HttpClient(); // Destructor implementation

    // Low-level: send a full URL (e.g. "http://host:port/path"). Returns true for 2xx
    bool sendRequest(const std::string& url);

    // Convenience: perform a GET to the configured host:port and the given path.
