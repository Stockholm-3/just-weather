...
static void* server_thread(void* arg) {
    ...
    struct sockaddr_in6 bnd = {0};
    socklen_t           bl  = sizeof(bnd);
...
TEST(tcp_client_ping_pong) {
    ...
    const char* msg = "ping\n";
    assert(tcp_client_write(&c, (const uint8_t*)msg, (int)strlen(msg)) ==
           (int)strlen(msg));
...
void tcp_client_all(void) { RUN_SUB_TEST(tcp_client_ping_pong); }
