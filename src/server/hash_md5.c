/**
 * hash_md5.c - Modular MD5 hashing implementation
 *
 * This file contains:
 * 1. Original Solar Designer's MD5 implementation (public domain)
...
/* ========================================================================
 * ORIGINAL MD5 IMPLEMENTATION BY ALEXANDER PESLYAK (SOLAR DESIGNER)
 * Public Domain -
...
int hash_md5_binary_to_string(const unsigned char* binary, char* output,
                              size_t output_size) {
    if (!binary || !output || output_size < HASH_MD5_STRING_LENGTH) {
        return -1;
    }

    /* Convert binary to hex string */
    for (int i = 0; i < HASH_MD5_BINARY_LENGTH; i++) {
        snprintf(&output[i * 2], 3, "%02x", (unsigned int)binary[i]);
    }
    output[HASH_MD5_STRING_LENGTH - 1] = '\0';
...
int hash_md5_string(const void* data, size_t data_size, char* output,
                    size_t output_size) {
    if (!data || !output || output_size < HASH_MD5_STRING_LENGTH) {
        return -1;
    }
    ...
    if (hash_md5_binary(data, data_size, binary) != 0) {
        return -1;
    }

    /* Convert to hex string */
    return hash_md5_binary_to_string(binary, output, output_size);
}
