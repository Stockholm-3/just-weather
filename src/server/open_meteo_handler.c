/*  open_meteo_handler.c -  open_meteo_handler - endpoint handler for HTTP server */
...
int open_meteo_handler_current(const char* query_string, char** response_json, int* status_code) {
    ...
    *status_code = HTTP_INTERNAL_ERROR;
    ...
    if (open_meteo_api_parse_query(query_string, &lat, &lon) != 0) {
        *response_json = build_error_response(
            "Invalid query parameters. Expected format: lat=XX.XXXX&long=YY.YYYY",
            HTTP_BAD_REQUEST
        );
...
void wopen_meteo_handler_cleanup(void) {
