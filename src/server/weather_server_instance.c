...
// --- Parse JSON into Meteo struct (including elevation) ---
int parse_weather_json(const char* json_text, Meteo* meteo) {
    ...
    // Elevation is at root level
    json_t* elev = json_object_get(root, "elevation");
    if (json_is_number(elev)) {
        meteo->elevation = json_number_value(elev);
    } else {
        meteo->elevation = 0.0;
    }
    ...
    const char* current_time =
        json_string_value(json_object_get(current_weather, "time"));
    meteo->temperature =
        json_number_value(json_object_get(current_weather, "temperature"));
    meteo->windspeed =
        json_number_value(json_object_get(current_weather, "windspeed"));
    meteo->winddirection =
        json_number_value(json_object_get(current_weather, "winddirection"));
    meteo->weathercode =
        json_integer_value(json_object_get(current_weather, "weathercode"));
    strncpy(meteo->time, current_time, sizeof(meteo->time) - 1);
...
void print_weather_as_json(double latitude, double longitude, Meteo* meteo) {
    ...
    json_object_set_new(current, "temperature_c",
                        json_real(meteo->temperature));
    ...
    json_object_set_new(current, "weather_code",
                        json_integer(meteo->weathercode));
    json_object_set_new(current, "elevation_m", json_real(meteo->elevation));
...
void test_fetch_and_parse_url() {
    ...
    const char* url =
        ...
        "forecast?latitude=37.7749&longitude=-122.4194&current_weather=true";
