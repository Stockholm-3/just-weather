...
int LLVMFuzzerTestInput(const uint8_t *data, size_t size){
    ...
    HTTPServerConnection* connection = http_server_connection_create_from_data(data, size);
    if (!connection) {
    ...
    WeatherServerInstance* weather_server_instance_create(){
        WeatherServerInstance* instance = (WeatherServerInstance*)malloc(sizeof(WeatherServerInstance));
        if(instance){
            instance->connection = NULL;
        }
        return instance;
    }
    HTTPServerConnection* http_server_connection_create_from_data(const uint8_t *data, size_t size){
