...
int parse_string(const char *input, Parser *out)
{
    ...
    if (!scheme_sep) return -2;

    // protocol
    size_t proto_len = (size_t)(scheme_sep - input);
    if (proto_len == 0 || proto_len >= sizeof(out->protocol)) return -3;
    if (copy_bounded(out->protocol, sizeof(out->protocol), input, proto_len) != 0) return -3;
    ...
    if (domain_len == 0 || domain_len >= sizeof(out->domain)) return -4;
    if (copy_bounded(out->domain, sizeof(out->domain), p, domain_len) != 0) return -4;

    // path = с первого '/' и дальше, либо "/" если его не было
    if (slash) {
        size_t path_len = strlen(slash);
        if (path_len + 1 > sizeof(out->path)) return -5;
        memcpy(out->path, slash, path_len + 1);
    } else {
        if (copy_bounded(out->path, sizeof(out->path), "/", 1) != 0) return -5;
    }
